...
path_finetuned_ML_model=str(pathlib.Path(__file__).parent.absolute())+'/gpt2_model/'

finetuned_ML_model= False  #If do have a fine-tuned model
...
repetition_penalty = 1.4 #Repetition Penalty drift
...
class MLdriftFallback(FallbackSkill):
    ...
    def handle_One(self, blabla):
        ...
        generator = self.model.generate(process, max_length = lengthDrift, temperature= temperature, repetition_penalty = repetition_penalty, do_sample=True, top_k=20)
        drift = self.tokenizer.decode(generator.tolist()[0])

        drift = drift.replace(str(blabla), "")
        self.log.info("GENERATED TEXT: "+ drift)
